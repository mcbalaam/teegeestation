...
/obj/machinery/airalarm/crowbar_act(mob/living/user, obj/item/tool)
	if(buildstage != AIR_ALARM_BUILD_NO_WIRES)
		return
	user.visible_message(span_notice("[user.name] извлекает плату из [declent_ru(GENITIVE)]."), \
						span_notice("Вы начинаете извлекать плату..."))
	tool.play_tool_sound(src)
	if (tool.use_tool(src, user, 20))
		if (buildstage == AIR_ALARM_BUILD_NO_WIRES)
			to_chat(user, span_notice("Вы извлекли плату воздушной сигнализации."))
...
/obj/machinery/airalarm/screwdriver_act(mob/living/user, obj/item/tool)
	...
	tool.play_tool_sound(src)
	toggle_panel_open()
	to_chat(user, span_notice("Проводка [panel_open ? "раскрыта" : "спрятана"]."))
	update_appearance()
	return TRUE
...
/obj/machinery/airalarm/wrench_act(mob/living/user, obj/item/tool)
	if(buildstage != AIR_ALARM_BUILD_NO_CIRCUIT)
		return
	to_chat(user, span_notice("Вы открепили [declent_ru(ACCUSATIVE)] от стены."))
	tool.play_tool_sound(src)
...
/obj/machinery/airalarm/proc/togglelock(mob/living/user)
	if(machine_stat & (NOPOWER|BROKEN))
		to_chat(user, span_warning("Бесполезно!"))
	else
...
/obj/machinery/airalarm/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	balloon_alert(user, "датчики аутентификации сожжены")
		...
		if(AIR_ALARM_BUILD_NO_WIRES)
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(cable.get_amount() < 5)
					...
					return
				user.visible_message(span_notice("[user.name] подключает воздушную сигнализацию."), \
									span_notice("Вы начинаете прокладывать проводку..."))
				if (do_after(user, 2 SECONDS, target = src))
					if (cable.get_amount() >= 5 && buildstage == AIR_ALARM_BUILD_NO_WIRES)
						cable.use(5)
						to_chat(user, span_notice("Вы подключили воздушную сигнализацию."))
						wires.repair()
						aidisabled = FALSE
						locked = FALSE
						shorted = FALSE
		...
		if(AIR_ALARM_BUILD_NO_CIRCUIT)
			if(istype(W, /obj/item/electronics/airalarm))
				if(user.temporarilyRemoveItemFromInventory(W))
					to_chat(user, span_notice("Вы вставили плату."))
					buildstage = AIR_ALARM_BUILD_NO_WIRES
					update_appearance()
					qdel(W)
				return
			...
			if(istype(W, /obj/item/electroadaptive_pseudocircuit))
				...
				if(!P.adapt_circuit(user, circuit_cost = 0.025 * STANDARD_CELL_CHARGE))
					return
				user.visible_message(span_notice("[user] изготавливает плату и вставляет её в [declent_ru(ACCUSATIVE)]."), \
				span_notice("Вы адаптируете плату воздушной сигнализации и вставляете её в корпус."))
				buildstage = AIR_ALARM_BUILD_NO_WIRES
				update_appearance()
				return
...
/obj/item/wallframe/airalarm
	...
	desc = "Используется для сборки воздушной сигнализации."
