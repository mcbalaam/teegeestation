/obj/machinery/airalarm/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(held_item.tool_behaviour == TOOL_CROWBAR)
		...
		context[SCREENTIP_CONTEXT_LMB] = panel_open ? "Скрыть проводку" : "Раскрыть проводку"

	else if(held_item.tool_behaviour == TOOL_WIRECUTTER)
		if (panel_open)
			context[SCREENTIP_CONTEXT_LMB] = "Манипулировать проводкой"

	else if(held_item.tool_behaviour == TOOL_WRENCH)
		if(buildstage == AIR_ALARM_BUILD_NO_CIRCUIT)
			context[SCREENTIP_CONTEXT_LMB] = "Снять сигнализацию"
