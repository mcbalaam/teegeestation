...
/obj/machinery/disposal/attackby(obj/item/I, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if(!pressure_charging && !full_pressure && !flush)
		if(I.tool_behaviour == TOOL_SCREWDRIVER)
			...
			to_chat(user, span_notice("Вы [panel_open ? "откручиваете":"закручиваете"] винты вокруг крепления."))
			return
...
/obj/machinery/disposal/proc/rat_rummage(mob/living/basic/regal_rat/king)
	king.visible_message(span_warning("[king] начинает рыться в [declent_ru(GENITIVE)]."),span_notice("Вы начинаете рыться в [declent_ru(GENITIVE)]..."))
	...
	switch(loot)
		if(1 to 5)
			to_chat(king, span_notice("Вы находите кучку монет. В королевскую казну!"))
			var/pickedcoin = pick(GLOB.ratking_coins)
			for(var/i = 1 to rand(1,3))
				new pickedcoin(king.drop_location())
		if(6 to 33)
			cheese = TRUE
			king.say(pick("Сокроооовищееее!","Наше драгоцеееенное!","Сыыыырррр!"), ignore_spam = TRUE, forced = "regal rat rummaging")
			to_chat(king, span_notice("Есть! Вы находите сыр!"))
			new /obj/item/food/cheese/wedge(king.drop_location())
		else
			var/pickedtrash = pick(GLOB.ratking_trash)
			to_chat(king, span_notice("Вы находите лишь мусор и грязь. Неплохо, но теперь это ниже вашего достоинства."))
	...
	if (LAZYLEN(SSmobs.cheeserats) < rat_cap && prob(33))
		...
		visible_message(span_warning("[new_subject] вылезает из [declent_ru(GENITIVE)]!"))
...
/obj/machinery/disposal/proc/place_item_in_disposal(obj/item/disposing_item, mob/user)
	...
	user.visible_message(
		span_notice("[user.name] кладёт [disposing_item.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
		span_notice("Вы кладёте [disposing_item.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
	)
	return TRUE
...
/obj/machinery/disposal/proc/stuff_mob_in(mob/living/target, mob/living/user)
	...
	if(target.mob_size > MOB_SIZE_HUMAN)
		to_chat(user, span_warning("[target] не помещается в [declent_ru(ACCUSATIVE)]!"))
		return FALSE
	add_fingerprint(user)
	if(user == target)
		user.visible_message(span_warning("[user] начинает залезать в [declent_ru(ACCUSATIVE)]."), span_notice("Вы начинаете залезать в [declent_ru(ACCUSATIVE)]..."))
	else
		target.visible_message(span_danger("[user] начинает заталкивать [target] в [declent_ru(ACCUSATIVE)]."), span_userdanger("[user] начинает заталкивать вас в [declent_ru(ACCUSATIVE)]!"))
	if(!do_after(user, 2 SECONDS, target) || QDELETED(src))
		return FALSE
	target.forceMove(src)
	if(user == target)
		user.visible_message(span_warning("[user] залезает в [declent_ru(ACCUSATIVE)]."), span_notice("Вы залезаете в [declent_ru(ACCUSATIVE)]."))
	else
		target.visible_message(span_danger("[user] помещает [target] в [declent_ru(ACCUSATIVE)]."), span_userdanger("[user] помещает вас в [declent_ru(ACCUSATIVE)]."))
...
/obj/machinery/disposal/proc/stuff_bodybag_in(obj/structure/closet/body_bag/bag, mob/living/user)
	if(!length(bag.contents))
		...
		qdel(bag)
		user.visible_message(
			span_warning("[user] заталкивает пустой [bag.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
			span_notice("Вы заталкиваете пустой [bag.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)].")
		)
		return TRUE

	user.visible_message(
		span_warning("[user] начинает заталкивать [bag] в [declent_ru(ACCUSATIVE)]."),
		span_notice("Вы начинаете заталкивать [bag] в [declent_ru(ACCUSATIVE)]...")
	)
	...
	user.visible_message(
		span_warning("[user] помещает [bag] в [declent_ru(ACCUSATIVE)]."),
		span_notice("Вы помещаете [bag] в [declent_ru(ACCUSATIVE)].")
	)
...
/obj/machinery/disposal/proc/on_storage_dump(datum/source, datum/storage/storage, mob/user)
	...
	to_chat(user, span_notice("Вы высыпаете [storage.parent.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."))
...
/obj/machinery/disposal/proc/trash_living(datum/source, mob/living/shover, mob/living/target, shove_flags, obj/item/weapon)
	...
	target.forceMove(src)
	target.visible_message(span_danger("[shover.name] заталкивает [target.name] в [declent_ru(ACCUSATIVE)]!"),
		span_userdanger("Вас заталкивают в [declent_ru(ACCUSATIVE)] [shover.name]!"), span_hear("Вы слышите агрессивную возню, а затем громкий грохот!"), COMBAT_MESSAGE_RANGE, shover)
	to_chat(shover, span_danger("Вы заталкиваете [target.name] в [declent_ru(ACCUSATIVE)]!"))
...
/obj/machinery/disposal/bin
	name = "disposal unit"
	desc = "Пневматический утилизационный мусорный блок."
	icon_state = "disposal"
...
/obj/machinery/disposal/bin/attackby_secondary(obj/item/weapon, mob/user, list/modifiers, list/attack_modifiers)
	if(istype(weapon, /obj/item/dest_tagger))
		...
		if(mounted_tagger)
			balloon_alert(user, "уже есть маркировщик!")
			return
		...
		user.visible_message(span_notice("[user] защёлкивает [new_tagger.declent_ru(ACCUSATIVE)] на [declent_ru(GENITIVE)]."))
		balloon_alert(user, "маркировщик установлен")
...
/obj/machinery/disposal/bin/attack_hand_secondary(mob/user, list/modifiers)
	. = ..()
	if(!mounted_tagger)
		balloon_alert(user, "нет маркировщика!")
		return SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN
	if(!user.put_in_hands(mounted_tagger))
		balloon_alert(user, "маркировщик падает!")
		mounted_tagger = null
		return SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN
	user.visible_message(span_notice("[user] отцепляет [mounted_tagger.declent_ru(ACCUSATIVE)] от [declent_ru(GENITIVE)]."))
...
/obj/machinery/disposal/bin/examine(mob/user)
	...
	if(isnull(mounted_tagger))
		. += span_notice("Крепление для маркировщика пусто.")
	else
		. += span_notice("[mounted_tagger] висит сбоку. ПКМ чтобы снять.")
...
/obj/machinery/disposal/bin/hitby(atom/movable/AM, skipcatch, hitpush, blocked, datum/thrownthing/throwingdatum)
	if(isitem(AM) && AM.CanEnterDisposals())
		...
		if((istype(thrower) && HAS_TRAIT(thrower, TRAIT_THROWINGARM)) || prob(75))
			AM.forceMove(src)
			visible_message(span_notice("[AM] приземляется в [declent_ru(ACCUSATIVE)]."))
			update_appearance()
		else
			visible_message(span_notice("[AM] отскакивает от края [declent_ru(GENITIVE)]!"))
			return ..()
	else
		return ..()
...
/obj/machinery/disposal/bin/proc/release_animal()
	...
	visible_message(span_notice("Испуганный [startled_animal] выпрыгивает из [declent_ru(GENITIVE)]."))
...
/obj/machinery/disposal/delivery_chute
	name = "delivery chute"
	desc = "Жёлоб для больших и маленьких посылок!"
	density = TRUE
	icon_state = "intake"
...
/obj/machinery/disposal/delivery_chute/Bumped(atom/movable/AM) //Go straight into the chute
	...
	if(isobj(AM))
		...
		if(prob(2)) // to prevent mobs being stuck in infinite loops
			to_chat(M, span_warning("Вы ударяетесь о край жёлоба."))
			return
		M.forceMove(src)
	flush()
